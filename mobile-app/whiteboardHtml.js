...
<div id="toolbar">
  ...
  <div id="tools">
    ...
    <button id="t-fullscreen" title="Fullscreen"><div class="ic" id="fs-ic">⛶</div><div class="lb" id="fs-lb">Full</div></button>
...
(function(){
  ...
  function syncFsLabel(){
    const ic=document.getElementById('fs-ic'); const lb=document.getElementById('fs-lb');
    if(isFs()){ ic.textContent='✕'; lb.textContent='Exit'; } else { ic.textContent='⛶'; lb.textContent='Full'; }
    setTimeout(fitCanvas,80);
  }
  ['fullscreenchange','webkitfullscreenchange','mozfullscreenchange'].forEach(ev=>document.addEventListener(ev,syncFsLabel));
  document.getElementById('t-fullscreen').onclick=function(){
    try{
      if(isFs()){
        ...
        if(req) req.call(el);
          ...
          // iframe context: ask parent to fullscreen the iframe
          window.parent.postMessage({type:'wb-fullscreen'},'*');
        } else { showToast('Fullscreen not supported here'); }
      }
    }catch(err){ showToast('Fullscreen blocked: '+(err.message||err)); }
  };
